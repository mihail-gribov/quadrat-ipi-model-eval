...
Needs matplotlib (`pip install -e '.[figures]'`); nothing else in the harness does.
    ...
    python3 figure.py            # published sweeps -> reports/figures/money7-models.{png,svg}
...
def write(recs, slug, out_dir=REPORTS):
    """The figure for one set of sweeps: `<out_dir>/figures/<slug>-models.{png,svg}`.
    Returns the paths, or None with a note when matplotlib is missing."""
    try:
    ...
    if not rows:
        return None
    outs = [out_dir / "figures" / f"{slug}-models.png", out_dir / "figures" / f"{slug}-models.svg"]
    draw(rows, meta, outs)
    return outs
